...
// limpiar filas con Pedido vacío o NULL
if (isset($conn_pedidos) && $conn_pedidos instanceof mysqli) {
    $conn_pedidos->query("DELETE FROM pedidos WHERE Pedido IS NULL OR TRIM(Pedido) = ''");

    // eliminar filas con JSON inválido
    $res = $conn_pedidos->query("SELECT id, Pedido FROM pedidos");
    if ($res) {
        $delStmt = $conn_pedidos->prepare("DELETE FROM pedidos WHERE id = ?");
        while ($r = $res->fetch_assoc()) {
            $decoded = json_decode($r['Pedido'], true);
            if ($decoded === null) {
                $delStmt->bind_param('i', $r['id']);
                $delStmt->execute();
            }
        }
...
$orders = [];
if (isset($conn_pedidos) && $conn_pedidos instanceof mysqli) {
    // devolver solo pedidos que NO estén en 'Listo' ni 'Entregado'
    $q = $conn_pedidos->query("SELECT id, Pedido, Estado, created_at FROM pedidos WHERE Estado NOT IN ('Listo','Entregado') ORDER BY id ASC");
    if ($q) {
                ...
                <div class="list-group-item">
                  ...
                  <div class="d-flex justify-content-between align-items-center">
                    ...
                    <div class="d-flex gap-2">
                      <form method="post" action="cambiar_estado.php" class="d-flex gap-2 align-items-center">
                        <input type="hidden" name="id" value="<?= intval($ord['db_id']) ?>">
                        <select name="estado" class="form-select form-select-sm">
                          <option value="En Preparacion" <?= $ord['estado'] === 'En Preparacion' ? 'selected' : '' ?>>En Preparacion</option>
                          <option value="Listo" <?= $ord['estado'] === 'Listo' ? 'selected' : '' ?>>Listo</option>
