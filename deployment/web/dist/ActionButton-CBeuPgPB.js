import { getUI, getData, methodToPromise, AssertionError, getByGuid, MxObject, evaluateActionExpression, toNumber, mapValues, LogManager, ensure, getSession, crash, ValidationError, handleError, onlineData, isOffline, offlineData, getByGuids, translate, cancelChanges, nanoflowEngine, toNanoflowArguments, toRuntimeArguments, DescribedError, asProperty, useValue, fromComputation$, pageScope, partitionValues, getActionArguments$, attributeValueToActionArgument, action, readUntracked, runSideEffect, runInAction, subtract, assertIsNonEnumValue, getOrCrashL, filterValues, HandledError, toFriendlyId, mapL, loading, withFormSuspension$, fromRequest$, getSubscribedObject$, asDynamicValue$, toObjectItem, fromConstant$, withSideEffect$, asResult, dynAvailable, jsxRuntimeExports, reactExports, classNames, ActionContext } from './index-_VvUQn72.js';
...
async function openPage(modelerConfig, clientConfig, actionArguments) {
  ...
  for (const [name, value] of Object.entries(pageParameters))
    if (value)
      if ("object" in value)
        ...
        "primitive" in value && pageArguments.forEach((p) => p[name] = value.primitive);
      }
    else
  ...
  const formsOpened = await Promise.all(pageArguments.map(async function(args) {
    ...
    if (modelerConfig.specializations) {
      const value = args[modelerConfig.specializations.argument];
      value instanceof MxObject && (specializedSettings = modelerConfig.specializations.pageSettings[value.getEntity()]);
    }
    const settings = specializedSettings ?? modelerConfig, title = settings.title ? await evaluateActionExpression(settings.title, parameters) ?? "" : void 0, numberOfPagesToClose = modelerConfig.numberOfPagesToClose ? toNumber(await evaluateActionExpression(modelerConfig.numberOfPagesToClose, parameters)) : void 0, formParameters = settings.params ? mapValues(settings.params, (p) => getFormParameterValue(args[p])) : mapValues(args, (o) => getFormParameterValue(o));
    function getFormParameterValue(arg) {
      return arg instanceof MxObject ? arg.getGuid() : arg;
    }
      ...
      case "callMicroflow":
        return async function(modelerConfig, clientConfig2, actionArguments2) {
          ...
          try {
            ...
            isOffline() ? await offlineData().executeMicroflow(modelerConfig.operationId, toRuntimeArguments(actionArguments2), clientConfig2.currentForm, validationGuids, Boolean(modelerConfig.async)) : await onlineData().executeMicroflow(modelerConfig.operationId, toRuntimeArguments(actionArguments2), clientConfig2.currentForm, validationGuids, Boolean(modelerConfig.async));
          } finally {
      ...
      case "saveChanges":
        return async function(config, { currentForm }) {
          ...
          try {
            ...
            isOffline() ? await new Promise((resolve, reject) => {
      ...
      case "deleteObject":
        return async function(config, { currentForm }, parameter) {
          ...
          if (await new Promise((resolve) => ui.confirmation({ content: confirmation, handler: () => {
            ...
            try {
              isOffline() ? await offlineData().delete(objects) : config.operationId && await onlineData().executeObjectAction(config.operationId, "delete", objects), config.closePage && await methodToPromise(currentForm, "close");
            } finally {
              currentForm.setSuspend(false);
            }
          }
...
class ActionButton extends reactExports.Component {
  ...
  deriveState(props, state) {
    ...
    return actionValue && !actionValue.canExecute ? { disabled: true, showDisabled: true } : actionValue?.isExecuting && actionValue.disabledDuringExecution ? (this.disableTimeout ?? (this.disableTimeout = window.setTimeout(() => {
...
export { ActionButton, ActionProperty, Icon, TextProperty, computeAction, executeAction, fromObjectProperty$, getParentIsReady$ };
