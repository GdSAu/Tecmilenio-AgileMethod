...
const logger$2 = LogManager.get().getLogger(), OfflineDatabaseObjectListProperty = asProperty((config, store, widgetId, propertyPath) => {
  const queryArguments$ = Object.fromEntries(Object.entries(config.arguments ?? {}).map(([runtimeName, [source, attribute, isLocalVariable]]) => tuple(runtimeName, isLocalVariable ? localVariableRuntimeValue$(runtimeName, { widget: source, source: "primitive" }, store) : void 0 === attribute ? getSubscribedObjectState$(store, source) : getSubscribedAttribute$(store, source, attribute)))), constraints = config.constraints, fetchParameters$ = constraints ? fromComputation$(`queryFilter for ${widgetId}/${propertyPath}`, () => {
...
const DatabaseObjectListProperty = (config) => isOffline(config.entity) ? OfflineDatabaseObjectListProperty(config) : OnlineDatabaseObjectListProperty(config);
...
export { DatabaseObjectListProperty, listItemProperty, registerSortingAttribute, withFirstLoadDoneSharing$ };
