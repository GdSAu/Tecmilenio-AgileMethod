...
const Migrations = [(lowLevelDatabaseAPI) => {
  ...
  const scopedAssociationTable = toUserScopedName(ASSOCIATION_TABLE);
  await lowLevelDatabaseAPI.inTransaction(async (tx) => {
    offlineMetadatas.flatMap((sessionMetadata) => {
      const associationFields = Object.entries(sessionMetadata.attributes).filter(([, attr]) => "ObjectReference" === attr.type);
      return 0 === associationFields.length ? [] : [[`INSERT INTO "${scopedAssociationTable}"`, `(${PARENT_GUID_COLUMN}, ${CHILD_GUID_COLUMN}, ${ASSOCIATION_NAME_COLUMN})`, associationFields.map(([name]) => `SELECT "${GUID_COLUMN}", "${name}", '${name}'
                                 FROM "${toUserScopedName(toSafeKey(sessionMetadata.objectType))}"
                                 WHERE "${name}" IS NOT NULL`).join(" UNION ")].join(" "), ...associationFields.flatMap(([name]) => [`DROP INDEX IF EXISTS "idx_${toUserScopedName(toSafeKey(sessionMetadata.objectType))}_${toSafeKey(name)}"`, `ALTER TABLE "${toUserScopedName(toSafeKey(sessionMetadata.objectType))}" DROP COLUMN "${toSafeKey(name)}"`])];
    }).forEach((query) => tx.exec(query));
  });
...
async function synchronizeObjects({ dirtyMxObjects, syncedObjsOfflineToRuntimeMap, returnObjects, deletes = {}, extraGuidsToRetrieve = [], incompatibleObjectsJson = [], logger, database, fileBackend }) {
  ...
  logger.trace(`Created guids for ${offlineObjects.length} offline objects in ${stopwatch.measure()} milliseconds.`), stopwatch.reset(), await async function(offlineMxobjs, offlineToRuntimeGuids, fileBackend2, database2) {
    ...
    await database2.usingWriteTransaction(async (tx) => {
      await tx.updateObjectGuids(new GuidMappingWithEntity(offlineToRuntimeGuids.entries(), (guid) => objsMap[guid]?.getEntity()));
    });
  ...
  }(offlineObjects, createdGuidsMapping, fileBackend, database), logger.trace(`Updated offline guids with runtime guids in ${stopwatch.measure()} milliseconds.`);
...
async function deleteObjects(objectsToDelete, objectCache, fileBackend, schema, excludeEntitiesFromSync, database) {
  ...
  await database.usingWriteTransaction(async (tx) => {
    await tx.deleteMxObjects(peObjects.map((obj) => allOfflineToRuntimeGuidsMap.map(obj.getGuid())));
...
async function handleOfflineResponseObjects(runtimeGuidsToHandle, database, responseObjects, objectCache, offlineEntities, syncedObjsRuntimeToOfflineMap2, instantiatedObjectsRuntimeToOfflineMap = new RuntimeToOfflineGuidMapping(), tombstoneTableAction = "nothing") {
  ...
  await database.usingWriteTransaction(async (tx) => {
    if (runtimeGuidsToHandle.length > 0) {
      ...
      }), await tx.deleteMxObjects(toDelete), await tx.insertOrUpdateMxObjects(toInsert), "clean" === tombstoneTableAction && toInsert.length > 0) {
  ...
  try {
    const [changes, objects] = function([changes2, objects2]) {
      ...
      function filterChange(guid, change) {
        ...
        return Object.fromEntries(Object.entries(change).filter(([attr, value]) => !isAssociationToUnsyncedOffline(entity, attr, value.value ?? null)));
      }
    }(objectCache, changes);
...
function isAssociationToUnsyncedOffline(entity, attribute, value) {
  ...
  return void 0 !== attributeDef && ("ObjectReference" === attributeDef.type || "ObjectReferenceSet" === attributeDef.type) && null !== value && !isRuntimeGuid(value);
}
    ...
    return await async function(database2, fileBackend2, cache2, { schema, preserveData, fetch, never, online }, uploadedObjsEntityToGuidsMap2, resetPreservedEntities2, incompatibleEntities2, logger2) {
      ...
      return logger2.trace(`Computing subscription updates completed in ${stopwatch.measure()} milliseconds.`), logger2.trace("Rebuilding the database."), stopwatch.reset(), await async function(database3, schema2, objs, entityToGuidsMapToDelete, entitiesToPreserve, excludedEntities, incompatibleEntities3 = /* @__PURE__ */ new Set()) {
        ...
        await database3.usingWriteTransaction(async (tx) => {
          if (fullReset)
            await tx.reset(), await tx.updateEntityMetadatas(getSession().getOfflineMetadata());
          else {
            const entitiesToDrop = [...entitiesToReset, ...incompatibleEntities3];
            await tx.dropEntityTables(entitiesToDrop), await tx.deleteMxObjects(Object.values(entityToGuidsMapToDelete).flat());
          }
...
async function selectiveSync(objects, deletesToSync, offlineEntities, objectCache, database, fileBackend, logger) {
  ...
  const stopwatch = new Stopwatch(), updates = await runInSynchronizationLock(async () => {
    ...
    stopwatch.reset();
    const { createdGuidsMapping, response } = await synchronizeObjects({ dirtyMxObjects: dirtyObjects, syncedObjsOfflineToRuntimeMap, returnObjects: true, deletes: deletesToSync, extraGuidsToRetrieve: alreadySynchronizedMaybeOfflineGuids, logger, database, fileBackend });
...
class OfflineData {
  ...
  async insertOrReplace(objs) {
    ...
    await this.database.usingWriteTransaction(async (tx) => {
      ...
      return tx.insertOrUpdateMxObjectsWithSyncId(objs.map((obj, idx) => ({ ...obj, syncId: syncIds[idx] })));
    });
  }
