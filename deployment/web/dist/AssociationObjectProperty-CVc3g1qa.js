import { LogManager, asProperty, getSubscribedObjectState$, ensure, toFriendlyId, isOffline, getObjectByPath$, available, MxObject, getOrCrashL, readUntracked, onlineData, unavailable } from './index-D54rfXjv.js';
import { fromObjectProperty$ } from './ActionButton-Cg_9OV9E.js';
...
const logger = LogManager.get().getLogger(), AssociationObjectProperty = asProperty((config, store, widgetId) => {
  ...
  return !config.path || isOffline() ? fromObjectProperty$(config, store, widgetId, config.path ? getObjectByPath$(rootObject$, config.path, store) : rootObject$, (p) => (logUsingObject(p), available(p))) : fromObjectProperty$(config, store, widgetId, getObjectByPath$(rootObject$, config.path, store, true), (parameters) => {
