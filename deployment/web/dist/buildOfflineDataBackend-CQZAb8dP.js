import { METADATA_TABLE, SYNC_ID_COLUMN, GUID_COLUMN, getSession, createUpdateCounterValue, createGetDbMetaValue, DB_COUNTER_KEY, ensure, DB_VALUE_COLUMN, DB_META_TABLE, getUserScopedNamePrefix, toUserScopedName, INACCESSIBLE_COLUMN, toSafeKey, createAlterTableQueries, CHILD_GUID_COLUMN, PARENT_GUID_COLUMN, createChunkedQuery, repeat, createRefererenceSetTableSql, createDropTableSqlQuery, createCheckTableExistSqlQuery, createMemoizedCache, isFileDocument, indexByKey, partition, isDirtyGuid, isImage, getRemoteDynamicResourceUrl, Stopwatch, upload, wait, AssertionError, ConnectionError, isRuntimeGuid, OfflineToRuntimeGuidMapping, createGuids, indexMxObjectsByGuid, syncedObjsRuntimeToOfflineMap, isFileDocumentWithContents, isFileDocumentWithContentNonMetaCheck, createChange, createChangeFromIncompatibleObjectsJson, synchronizeObjects as synchronizeObjects$1, uniqueBy, offlineData, DescribedError, handleError, unique, getEntityAssociationsReferringTo, clone, unmarkAsDirty, publish, LogManager, LogNode, indexMxObjectJsonsByGuid, RuntimeToOfflineGuidMapping, gatherUpdates, deriveEntity, getRequestDataForMicroflow, getGuidsFromRuntimeArguments, getImplicitMicroflowParameters, executeAsync, runtimeOperation, ValidationError, executeNoImpactInstructions, handleValidations, handleRuntimeSuccessResponse, runtimeValueToExpressionVariable, getEntityMeta, retrieveByXPath, retrieveByIds, SystemAttribute, SynchronizationError, runInSynchronizationLock, actionMatcher, getRemoteUrl, registerMiddleware, createRetryMiddlewarePredicate, markAsDirty, createOfflineGuid, createMendixGuid, createMxObjectJSON, MxObject, clearDirtyGuids, _DataBackend, getAssociationStep, getCachedObjectsByPath, getRuntimeGuid, getRefsObjects, objectFromArray, mapObject, memoizeConcurrent, getAppUrl, isReferenceType } from './index-_VvUQn72.js';

const Migrations = [(lowLevelDatabaseAPI) => {
  const [addColumnQuery, addColumnParams] = createAddColumnSqlQueries(METADATA_TABLE, { name: SYNC_ID_COLUMN, type: "text" });
  return lowLevelDatabaseAPI.inTransaction(async (tx) => {
    await tx.exec(addColumnQuery, [addColumnParams]);
    const dirtyGuids = (await tx.selectObjects(`SELECT "${GUID_COLUMN}"
                 FROM "${METADATA_TABLE}"
                 WHERE "dirty" = ?`, [1])).map((row) => row[GUID_COLUMN]), session = getSession(), syncIds = await async function(tx2, sessionId, count) {
    ...
    await Promise.all(dirtyGuids.map(async (guid, idx) => tx.exec(`UPDATE ${METADATA_TABLE}
                             SET ${SYNC_ID_COLUMN} = ?
                             WHERE ${GUID_COLUMN} = ?`, [ensure(syncIds[idx]), guid])));
  });
}, (lowLevelDatabaseAPI) => lowLevelDatabaseAPI.inTransaction(async (tx) => {
  const tablesToMigrate = (await lowLevelDatabaseAPI.selectObjects("SELECT name\n                 FROM sqlite_master\n                 WHERE type = 'table'\n                   AND name NOT LIKE 'sqlite_%'\n                   AND name != 'android_metadata' AND name != ?", [DB_META_TABLE])).map((r) => r.name.toString()).filter((n) => !n.startsWith(getUserScopedNamePrefix()));
  ...
  const [addColumnQuery, addColumnParams] = createAddColumnSqlQueries(toUserScopedName(METADATA_TABLE), { name: INACCESSIBLE_COLUMN, type: "text" });
  return lowLevelDatabaseAPI.inTransaction(async (tx) => {
    await tx.exec(addColumnQuery, [addColumnParams]), await tx.exec(`UPDATE ${toUserScopedName(METADATA_TABLE)} SET ${INACCESSIBLE_COLUMN} = '[]'`);
  });
}, async (lowLevelDatabaseAPI, offlineMetadatas) => {
  const scopedAssociationTable = toUserScopedName("_associationTable");
  async function getQueriesForReferenceFieldMigrations(tx, sessionMetadata) {
    const tableName = toUserScopedName(toSafeKey(sessionMetadata.objectType)), nonExistingReferenceFields = await async function(tx2, tableName2, attributes) {
      const referenceFields = Object.entries(attributes).filter(([, attr]) => "ObjectReference" === attr.type);
      if (0 === referenceFields.length)
        return [];
      const currentFieldRecords = await tx2.selectObjects("select name from pragma_table_info(?)", [tableName2]), currentFields = new Set(currentFieldRecords.map((r) => r.name));
      return referenceFields.filter(([name]) => !currentFields.has(toSafeKey(name)));
    }(tx, tableName, sessionMetadata.attributes), alterTableQueries = createAlterTableQueries(sessionMetadata.objectType, nonExistingReferenceFields, []), insertQueries = nonExistingReferenceFields.map(([name]) => [`UPDATE "${tableName}" SET "${toSafeKey(name)}" = "${scopedAssociationTable}"."${CHILD_GUID_COLUMN}"`, `FROM "${scopedAssociationTable}"`, `WHERE "${scopedAssociationTable}"."${PARENT_GUID_COLUMN}" = "${tableName}"."${GUID_COLUMN}" AND "${scopedAssociationTable}"."associationName" = '${name}'`].join(" ")), deleteQueries = createChunkedQuery(nonExistingReferenceFields.map(([name]) => name), (size) => `DELETE FROM "${toUserScopedName("_associationTable")}" WHERE "associationName" IN (${repeat(size, () => "?").join(", ")})`);
    return [...alterTableQueries, ...insertQueries.map((q) => [q, []]), ...deleteQueries];
  }
  function getQueriesForReferenceSetMigrations(sessionMetadata) {
    const referenceSetFields = Object.entries(sessionMetadata.attributes).filter(([, attr]) => "ObjectReferenceSet" === attr.type);
    return [...referenceSetFields.flatMap(([refSet]) => createRefererenceSetTableSql(refSet)), ...referenceSetFields.map(([refSet]) => [`INSERT INTO "${toUserScopedName(toSafeKey(refSet))}" (${PARENT_GUID_COLUMN}, ${CHILD_GUID_COLUMN}) SELECT ${PARENT_GUID_COLUMN}, ${CHILD_GUID_COLUMN} FROM "${scopedAssociationTable}" WHERE "${scopedAssociationTable}"."associationName" = '${refSet}'`, []])];
  }
  await lowLevelDatabaseAPI.inTransaction(async (tx) => {
    for (const sessionMetadata of offlineMetadatas) {
      const queries = [...await getQueriesForReferenceFieldMigrations(tx, sessionMetadata), ...getQueriesForReferenceSetMigrations(sessionMetadata)];
      for (const [query2, params2] of queries)
        await tx.exec(query2, params2);
    }
    const [query, params] = createDropTableSqlQuery(scopedAssociationTable);
    tx.exec(query, params);
  });
}].map((m, index) => ({ version: index + 1, apply: m }));
function createAddColumnSqlQueries(tableName, columnDescription) {
  return [`ALTER TABLE "${tableName}"
            ADD ${columnDescription.name} ${columnDescription.type} ${"text" === columnDescription.type ? "COLLATE NOCASE" : ""}`, []];
}
    ...
    await Promise.all(rows.map(async (row) => async function(wasqlite, webSqlDatabase, table, createTableStmt) {
      await wasqlite.inTransaction(async (tx) => {
        ...
        if (dataRows.length > 0) {
          const singleRowParameters = `(${Object.keys(dataRows[0]).map(() => "?").join(",")})`;
          await Promise.all(createChunkedQuery(dataRows.map((row2) => Object.values(row2)), (chunkSize) => `INSERT OR IGNORE INTO "${table}" VALUES ${repeat(chunkSize, () => singleRowParameters).join(", ")}`).map(async ([query, paramChunks]) => tx.exec(query, paramChunks)));
        }
        await executeSql([`DROP TABLE "${table}"`, []]).write(webSqlDatabase);
      });
...
class GuidMappingWithEntity {
  constructor(mapping, getEntityFun) {
    this.mapGuid = {}, this.mapEntity = {}, mapping.forEach(([from, to]) => {
      this.mapGuid[from] = to;
      const entity = getEntityFun(from) ?? getEntityFun(to);
      this.mapEntity[from] = ensure(entity);
    });
  }
  has(guid) {
    return void 0 !== this.mapGuid[guid];
  }
  get(guid) {
    return this.mapGuid[guid];
  }
  getGuid(guid) {
    return this.get(guid);
  }
  entries() {
    return Object.keys(this.mapGuid).map((guid) => [guid, this.mapGuid[guid] ?? null, this.mapEntity[guid]]);
  }
  getEntity(guid) {
    return this.mapEntity[guid];
  }
  getAllEntities() {
    return Object.values(this.mapEntity);
  }
  size() {
    return Object.keys(this.mapGuid).length;
  }
}
async function synchronizeObjects({ dirtyMxObjects, syncedObjsOfflineToRuntimeMap, returnObjects, deletes = {}, extraGuidsToRetrieve = [], incompatibleObjectsJson = [], logger, database, fileBackend, offlineEntities }) {
  ...
  logger.trace(`Created guids for ${offlineObjects.length} offline objects in ${stopwatch.measure()} milliseconds.`), stopwatch.reset(), await async function(offlineEntities2, offlineMxobjs, offlineToRuntimeGuids, fileBackend2, database2) {
    ...
    await database2.usingWriteTransaction(async (tx) => {
      await tx.updateObjectGuids(new GuidMappingWithEntity(offlineToRuntimeGuids.entries(), (guid) => objsMap[guid]?.getEntity()), offlineEntities2), syncedObjsRuntimeToOfflineMap.import(offlineToRuntimeGuids.reverse());
    });
    ...
    await executeFileInstructions({ moves }, fileBackend2);
...
async function deleteObjects(objectsToDelete, objectCache, fileBackend, schema, excludeEntitiesFromSync, database) {
  if (0 === objectsToDelete.length)
    return;
  const objects = uniqueBy(objectsToDelete, (obj) => obj.getGuid()), allOfflineToRuntimeGuidsMap = syncedObjsRuntimeToOfflineMap.reverse(), [runtimeObjects, peObjects, onlineObjectsMap] = objects.reduce(([runtimeObjectsArr, peObjectsArr, onlineObjMap], obj) => {
    !objectCache.isNew(obj.getGuid()) && isRuntimeGuid(allOfflineToRuntimeGuidsMap.map(obj.getGuid())) && runtimeObjectsArr.push(obj), obj.isPersistable() && peObjectsArr.push(obj);
    const entity = obj.getEntity();
    return offlineData().hasOnlineSyncMode(entity) && (onlineObjMap.has(entity) ? onlineObjMap.get(entity).push(obj.getGuid()) : onlineObjMap.set(entity, [obj.getGuid()])), [runtimeObjectsArr, peObjectsArr, onlineObjMap];
  }, [[], [], /* @__PURE__ */ new Map()]);
  ...
  await database.usingWriteTransaction(async (tx) => {
    await tx.deleteMxObjects(peObjects.map((obj) => allOfflineToRuntimeGuidsMap.map(obj.getGuid())), schema);
  ...
  executeFileInstructions({ deletes: peObjects.filter(({ jsonData }) => isFileDocumentWithContents(jsonData)).flatMap(({ jsonData }) => isImage(jsonData.objectType) ? [getFilePath(jsonData, false), getFilePath(jsonData, true)] : getFilePath(jsonData, false)) }, fileBackend).catch(handleError);
  const guidsToDelete = objects.map((obj) => obj.getGuid()), allOtherCachedObjects = objectCache.getAllObjects().filter((obj) => !allOfflineToRuntimeGuidsMap.has(obj.getGuid()) && !guidsToDelete.includes(obj.getGuid())), cachedNPEEntities = unique(allOtherCachedObjects.flatMap((obj) => obj.isPersistable() ? [] : obj.getEntity())), deletedEntitiesAllAssociationsPairsCache = createMemoizedCache((entity) => getEntityAssociationsReferringTo(entity, [...schema, ...cachedNPEEntities])), deletedEntities = unique(objects.map((obj) => obj.getEntity())), allAssociations = deletedEntities.flatMap((e) => Object.values(deletedEntitiesAllAssociationsPairsCache(e) ?? {})).flat(), objectRefsToDeletedObjectsPairs = allOtherCachedObjects.flatMap((obj) => {
  ...
  await database.usingWriteTransaction(async (tx) => {
    if (runtimeGuidsToHandle.length > 0) {
      ...
      if (runtimeGuidsToHandle.forEach((runtimeGuid) => {
        ...
        entity && isOfflineEntity(entity) && void 0 === objectToPush && toDelete.push(runtimeGuid), entity && isOfflineEntity(objectToPush?.objectType) && toInsert.push(objectToPush);
      }), await tx.deleteMxObjects(toDelete, offlineEntities), await tx.insertOrUpdateMxObjects(toInsert), "clean" === tombstoneTableAction && toInsert.length > 0) {
...
async function executeOfflineMicroflow(operationId, mfArgs, schema, objectCache, lockManager, database, fileBackend, targetForm, validationGuids = [], runAsync = false) {
  ...
  try {
    const [changes, objects] = function([changes2, objects2]) {
      const filteredChanges2 = Object.fromEntries(Object.entries(changes2).filter(([guid, _]) => isGuidOnlineOrNpe(guid))), filteredObjects2 = objects2.filter((object) => isGuidOnlineOrNpe(object.guid, object.objectType));
      return [filteredChanges2, filteredObjects2];
    }(getRequestDataForMicroflow(objectCache, operationId, getGuidsFromRuntimeArguments(mfArgs).concat(getImplicitMicroflowParameters()))), filteredObjects = function(objects2) {
      return function(objects3, filterAttribute) {
        return objects3.map((jsonData) => ({ ...jsonData, attributes: Object.fromEntries(Object.entries(jsonData.attributes).filter(([attr, value]) => filterAttribute(jsonData, attr, value))) }));
      }(objects2, (obj, attr, value) => !(void 0 === obj.hash && value.readonly || isAssociationToUnsyncedOffline(obj.objectType, attr, value.value)));
    }(objects), filteredChanges = function(objectCache2, changes2) {
      return Object.fromEntries(Object.entries(changes2).flatMap(([guid, change]) => {
        const remainingChange = filterChange(guid, change);
        return Object.keys(remainingChange).length > 0 ? [[guid, remainingChange]] : [];
      }));
      function filterChange(guid, change) {
        const entity = objectCache2.getObject(guid)?.getEntity() ?? deriveEntity(guid);
        return { ...change, members: Object.fromEntries(Object.entries(change.members).filter(([attr, value]) => !isAssociationToUnsyncedOffline(entity, attr, value.value ?? null))) };
      }
    }(objectCache, changes);
    runtimeResponse = runAsync ? await executeAsync((asyncId) => runtimeOperation(operationId, mfArgs, filteredChanges, filteredObjects, validationGuids, asyncId), 10) : await runtimeOperation(operationId, mfArgs, filteredChanges, filteredObjects, validationGuids);
  } catch (e) {
...
function isAssociationToUnsyncedOffline(entity, attribute, value) {
  const attributeDef = getEntityMeta(entity)?.attributes[attribute];
  return ("ObjectReference" === attributeDef?.type || "ObjectReferenceSet" === attributeDef?.type) && null !== value && !function(value2) {
    return Array.isArray(value2) ? value2.some((v) => isRuntimeGuid(v)) : isRuntimeGuid(value2);
  }(value);
}
...
async function downloadObjects(database, fileBackend, cache, syncConfig, uploadedObjsEntityToGuidsMap, resetPreservedEntities, logger, incompatibleEntities = /* @__PURE__ */ new Set()) {
  try {
    return await async function(database2, fileBackend2, cache2, { schema, preserveData, fetch, never, online }, uploadedObjsEntityToGuidsMap2, resetPreservedEntities2, incompatibleEntities2, logger2) {
      ...
      const [updatedObjs, guidsToDelete] = function(cache3, fetchedObjs2, entitiesToPreserve, entitiesToIgnore, uploadedObjsEntityToGuidsMap3, resetPreservedEntities3) {
        const syncedObjsOfflineToRuntimeMap = syncedObjsRuntimeToOfflineMap.reverse(), fetchedObjsWithCached = cache3.getAllObjects().filter((mxobj) => {
          if (!mxobj.isPersistable() || cache3.isNew(mxobj.getGuid()))
            return false;
          if (entitiesToIgnore.includes(mxobj.getEntity()))
            return false;
      ...
      }(cache2, fetchedObjsExcludingFailedDownloads, preserveData, never, uploadedObjsEntityToGuidsMap2, resetPreservedEntities2);
      ...
      return logger2.trace(`Computing subscription updates completed in ${stopwatch.measure()} milliseconds.`), logger2.trace("Rebuilding the database."), stopwatch.reset(), await async function(database3, schema2, objs, entityToGuidsMapToDelete, entitiesToPreserve, excludedEntities, incompatibleEntities3 = /* @__PURE__ */ new Set()) {
        ...
        await database3.usingWriteTransaction(async (tx) => {
          if (fullReset)
            await tx.dropAllTables(schema2), await tx.updateEntityMetadatas(getSession().getOfflineMetadata());
          else {
            const entitiesToDrop = [...entitiesToReset, ...incompatibleEntities3];
            await tx.dropEntityTables(entitiesToDrop), await tx.deleteMxObjects(Object.values(entityToGuidsMapToDelete).flat(), schema2);
          }
          await tx.ensureSchema(entitiesToReset, 0), await tx.insertOrUpdateMxObjects(objs);
        });
...
class OfflineData {
  constructor(syncConfig, objectCache, lockManager, fileBackend, database, logManager = LogManager.get()) {
    this.syncConfig = syncConfig, this.objectCache = objectCache, this.lockManager = lockManager, this.fileBackend = fileBackend, this.database = database, this.startupLogger = logManager.getLogger(LogNode.Startup), this.syncLogger = logManager.getLogger(LogNode.Synchronization), this.selectiveSyncLogger = logManager.getLogger(LogNode.SelectiveSync), this.onlineEntities = new Set(syncConfig.online), function() {
      ...
      function isDownload(request) {
        return "get" === request.init.method && request.url.startsWith(`${getRemoteUrl()}file?`);
      }
  ...
  async initialize(migrations, webSqlConnection) {
    ...
    }(migrations), dbVersion = await this.database.usingReadTransaction(async (tx) => await tx.checkTableExists(DB_META_TABLE) ? tx.getDatabaseVersion() : latestVersion);
    ...
    await this.database.usingWriteTransaction(async (tx) => {
      await tx.ensureSchema(tablesToInitialize, latestVersion);
    });
  ...
  async cleanDatabase() {
    await this.database.cleanDatabase(this.syncConfig.schema);
  }
  async create(entity) {
  ...
  async insertOrReplace(objs) {
    ...
    await this.database.usingWriteTransaction(async (tx) => {
      const offlineToRuntimeGuidMapping = syncedObjsRuntimeToOfflineMap.reverse(), syncIds = await generateSyncIds(tx, sessionId, objs.length);
      return tx.insertOrUpdateMxObjectsWithSyncId(objs.map((obj, idx) => ({ ...offlineToRuntimeGuidMapping.mapMxObjectJSON(obj), syncId: syncIds[idx] })));
    });
  }
  ...
  async upload(incompatibleEntities = /* @__PURE__ */ new Set()) {
    ...
    await this.database.usingWriteTransaction(async (tx) => [...await Promise.all(Object.entries(response.fileChangedDates).map(async ([runtimeGuid, changedDate]) => {
      ...
      return tx.updateObjectAttribute(mxobj.getEntity(), runtimeGuid, "changedDate", changedDate);
...
class OfflineDataBackend extends _DataBackend {
  ...
  async commit(commitGuids, context) {
    ...
    persistableCommitObjs.length > 0 && (await offlineData().insertOrReplace(persistableCommitObjs), markAsDirty(persistableCommitObjs.map((obj) => obj.guid)), this._objectCache.setMxObjects(persistableCommitObjs));
  ...
  async saveDocument(documentGuid, name, params, blob) {
    ...
    await this._fileBackend.storeFile(blob, this._fileBackend.toAbsolutePath("documents/" + fileName)), this._objectCache.makeChange(documentGuid, "HasContents", true), name && this._objectCache.makeChange(documentGuid, "Name", name), await this.commit([documentGuid], null);
  }
...
function getDefaultDocumentUrl(fileName, changedDate, isThumb) {
  ...
  return `filesystem:${getAppUrl()}temporary/${dir}/${fileName}?${Date.now()}`;
}
class SqlStore {
